...
public class CourseController {
  ...
  private CourseModel defineModelWithLinks(Course entity) {
    ...
    Link link = WebMvcLinkBuilder
        .linkTo(WebMvcLinkBuilder.methodOn(CourseController.class).getById(model.getCourseId()))
        .withSelfRel();
    model.add(link);
    return model;
  }
  ...
  public ResponseEntity<CourseModel> getById(@PathVariable("courseId") String courseId) {
    return courseRepository.findById(courseId).map(this::defineModelWithLinks)
